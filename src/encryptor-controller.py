...
class Encryptor:
    ...
    def __init__(self, event, context):
        ...
        self.is_default_s3_kms_key = os.environ.get('IsDefaultS3KMSKey')
    ...
    def __trigger_batch_job(self):
        ...
        if self.add_tag_to_objects == 'true':
    ...
    def update_resources(self):
        ...
        1. receive update
        2. check if 'encrypt' is now set to 'true'
            ...
            5. the s3 event notification will trigger the s3 batch encryption job, since
               the 'encrypt' variable is set to 'true' after the update
        6. if the 'encrypt' variable is set to 'false', nothing happens. s3 inventory reports
    ...
    def configure_resources(self):
        ...
        """
        if self.sse_type == 'SSE-KMS' and not self.is_default_s3_kms_key:
        ...
        if event['Records'][0]['s3']['bucket']['name'] == self.s3_inv_reports_bucket:
            ...
            if self.encrypt == 'true':
                ...
                info('new manifest was uploaded but a batch job was not triggered since the '
                     '"EncryptBuckets" parameter in the CloudFormation template is set to "false"')

            if self.remove_s3_inventory_config == 'true':
